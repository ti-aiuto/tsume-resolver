...
const KomaFactory = require('./koma-factory.js').KomaFactory;
...
exports.BanSnapshot = class BanSnapshot {
  ...
  toString() {
    ...
    [...BanPoint.sujiOptions()].reverse().forEach((suji) => {
      text += `  ${BanPoint.sujiLabel(suji)}  |`;
    });
    text += '\n';

    text += `${'-'.repeat(66)}\n`;
    BanPoint.danOptions().forEach((dan) => {
      text += `${BanPoint.danLabel(dan)}| `;
      [...BanPoint.sujiOptions()].reverse().forEach((suji) => {
        const banPoint = new BanPoint(suji, dan);
  ...
  asJson() {
    ...
    this.findOnBoardBanKomasBySide(sente).forEach((banKoma) => {
      json['initial_koma']['on_board']['sente'].push({
        suji: banKoma.banPoint.suji,
        dan: banKoma.banPoint.dan,
        nari: banKoma.nari,
        name: banKoma.koma.name(),
      });
    });
    this.findOnBoardBanKomasBySide(gote).forEach((banKoma) => {
  ...
  static buildFromJson(json) {
    ...
    json['initial_koma']['captured']['sente'].forEach((koma) =>
      banSnapshot.initAddCaptured(komaFactory.createKoma(koma['name']), sente),
    );

    json['initial_koma']['captured']['gote'].forEach((koma) =>
      banSnapshot.initAddCaptured(komaFactory.createKoma(koma['name']), gote),
    );
