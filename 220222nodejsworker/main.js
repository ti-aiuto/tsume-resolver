...
async function main() {
  ...
  const initialBanSnapshot = BanSnapshot.buildFromJson(json);
  ...
  while (true) {
    ...
    const resolver = new TsumeResolver(headNode, enemySide, depthLimit, true, 1);
    const foundTsumi = await resolver.resolve(true);
    if (foundTsumi) {
      break;
    }
    depthLimit += 2;
